...
class Controller:
    @staticmethod
    def saveTestCase(qid):
        GlobalVar.cursor.execute("SELECT test_id, input, output FROM que_ans WHERE qid = '%d'" % int(qid))
        testcase = GlobalVar.cursor.fetchall()
        testlist = dict()
        for data in testcase:
            file_name = GlobalVar.FILE_PATH_TESTCASE + "/" + str(data[0])
            file_in = file_name + ".in"
            ...
            file.write(data[2])
            file.close()
            testlist[data[0]] = file_name
        return testlist

    @staticmethod
    def getStuCode(id):
        GlobalVar.cursor.execute("SELECT compiler, answer, userid, qid FROM stu_que WHERE id = '%d'" % int(id))
        stucode = GlobalVar.cursor.fetchone()
        return stucode

    @staticmethod
    def updateState(id, state):
        GlobalVar.cursor.execute("UPDATE stu_que SET state = '%s' WHERE id = '%d'" % (str(state), int(id)))
        GlobalVar.db.commit()
    
    @staticmethod
    def getTimeMemory(qid):
        GlobalVar.cursor.execute("SELECT time, memory FROM question WHERE qid = '%d'" % int(qid))
        result = GlobalVar.cursor.fetchone()
        return int(result[0]), int(result[1])

    @staticmethod
    def compileError(id, message):
        GlobalVar.cursor.execute("UPDATE stu_que SET state = '4', result = '%s' WHERE id = '%d'" % (message, int(id)))
        GlobalVar.db.commit()

    @staticmethod
    def systemError(id, message):
        GlobalVar.cursor.execute("UPDATE stu_que SET state = '5', result = '%s' WHERE id = '%d'" % (message, int(id)))
        GlobalVar.db.commit()

    @staticmethod
    def updateResult(test_id, userid, state, time, memory):
        GlobalVar.cursor.execute("UPDATE stu_test SET state = '%s', time = '%d', memory = '%d' WHERE userid = '%d' AND test_id = '%d'" % (state, int(time), int(memory), int(userid), int(test_id)))
        GlobalVar.db.commit()
...
def judge(id, compiler, code, userid, qid):
    isPrint("judging!!! userid: %d, qid: %d" % (int(userid), int(qid)))
    ...
    if compiler == "gcc 7.5.0" or compiler == "gcc 8.4.0":
        ...
        Controller.systemError(id, "Unknow Language!")
        GlobalVar.statue = True
        return

    timelimit, memorylimit = Controller.getTimeMemory(qid)
    testlist = Controller.saveTestCase(qid)
    file_name_testcase = GlobalVar.FILE_PATH_TESTCASE + "/" + GlobalVar.judgername
    isAC = True

    for (test_id, testfile) in testlist.items():
        try:
            waittime = 0
            while True:
        ...
        if compiler == "gcc 7.5.0" or compiler == "gcc 8.4.0" or compiler == "g++ 7.5.0" or compiler == "g++ 8.4.0":
            ret = judgeC_CPP(timelimit, memorylimit, "%s.in" % testfile, "%stemp.out" % file_name_testcase, "%serror,out" % file_name_testcase)
        elif compiler == "python 2.7":
            ret = judgePython2(timelimit, memorylimit, "%s.in" % testfile, "%stemp.out" % file_name_testcase, "%serror,out" % file_name_testcase)
        elif compiler == "python 3.6":
            ret = judgePython3(timelimit, memorylimit, "%s.in" % testfile, "%stemp.out" % file_name_testcase, "%serror,out" % file_name_testcase)
        else:
            Controller.systemError(id, "Unknow Language!")
            GlobalVar.statue = True
            return
        ...
        if ret["result"] != 0:
            ...
            if (ret["result"] == 4 and ret["exit_code"] == 127 and ret["signal"] == 0) or (ret["result"] == 4 and ret["exit_code"] == 0 and ret["signal"] == 31):
                ...
                if ret["result"] == 1 or ret["result"] == 2:
                    ...
                    state = "System Error"
            Controller.updateResult(test_id, userid, state, test_time, test_memory)
        else:
            flag = 0   # ac: 0, wrong: 1, presentation: 2
            test_out = open("%s.out" % testfile, "r")
            user_out = open("%stemp.out" % file_name_testcase, "r")
            stdout = ""
            answer = ""
            while True:
                try:
                    ans = test_out.readline()
            ...
            Controller.updateResult(test_id, userid, state, test_time, test_memory)
    if isAC == True:
        Controller.updateState(id, 3)
    else:
        Controller.updateState(id, 4)
    GlobalVar.statue = True
            ...
            if data:
                if data == "getstatue":
                    ...
                    try:
                        ...
                        t = threading.Thread(target=judge, args=(tp[1], data[0], data[1], data[2], data[3]))
                        t.setDaemon(True)
                        t.start()
                    except:
